...
func newComponentsCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "components",
		Short: "Manage ProductBoard components",
	}

	cmd.AddCommand(newComponentsListCmd())
	cmd.AddCommand(newComponentsGetCmd())
...
func newComponentsListCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			items, err := c.GetList("/components", nil, limitFlag)
			if err != nil {
				handleError(err)
			}

			headers := []string{"ID", "Name", "Description", "Parent"}
			if err := output.Print(outputFormat, items, headers, func() [][]string {
				...
				for _, item := range items {
					rows = append(rows, []string{
						...
						output.SafeNested(item, "parent", "id"),
					})
				}
				return rows
			}); err != nil {
...
func newComponentsGetCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "get <id>",
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			item, err := c.GetSingle("/components/" + args[0])
			if err != nil {
				handleError(err)
			}
			...
			output.PrintSingleTable([][]string{
				...
				{"Parent ID", output.SafeNested(item, "parent", "id")},
			})
		},
	}
}
